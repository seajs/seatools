...
(function(global) {
  ...
  var report = (window.opener || window.top).totoro.report
  ...
  function getSuiteResult(result) {
    ...
    return {
      ...
      error: {count: (error.count - baseResult.error.count), suites: error.suites, message: error.message}
    }
  }
  ...
  var typeMapping = {
    ...
    'testEnd': function(result) {
      stats.tests++
      var passed = result.fail.count === 0 && result.error.count === 0
      ...
      if (passed) {
        ...
        stats.failures += (result.fail.count || result.error.count)
        status = 'fail'
        var msg = []
        ...
        if (result.error.count && result.error.message) {
          info.message = info.message + result.error.message
        }
      }
